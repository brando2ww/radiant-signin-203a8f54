-- Nota recebida vira conta a pagar sem digitação.
--
-- A varredura da SEFAZ (focusnfe-mde-sweep) já achava as notas: no Kōten
-- Garibaldi eram 100 notas paradas em "pendente", R$ 133.746,08 em compras que
-- nunca chegaram ao financeiro nem à DRE, a mais antiga de 06/07. O que faltava
-- era o elo: o resumo da SEFAZ não traz os itens, e o assistente de importação
-- exige os itens, então o operador não tinha por onde lançar.
--
-- Esta função usa só o cabeçalho, que o resumo sempre traz completo
-- (fornecedor, CNPJ, valor e data de emissão). O estoque continua entrando pelo
-- assistente quando o XML completo chegar.
...
begin
  ...
  if not (v_nota.user_id = v_actor or public.is_establishment_member(v_nota.user_id)) then
  ...
  -- Fornecedor: reaproveita pelo CNPJ, cria quando é a primeira compra.
  ...
  -- O resumo da SEFAZ não traz duplicata: sem vencimento informado, vale a data
  -- de emissão, que aparece vencida e puxa a conferência, em vez de inventar
...
grant execute on function public.pdv_lancar_nota_no_financeiro(uuid, date) to authenticated;
