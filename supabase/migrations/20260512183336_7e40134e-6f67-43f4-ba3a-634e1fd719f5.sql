CREATE OR REPLACE FUNCTION public.pdv_cancel_order(p_order_id uuid, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  v_owner uuid;
  v_order RECORD;
  v_now timestamptz := now();
BEGIN
  IF NOT public.has_pdv_action(auth.uid(), 'cancel_item') THEN
    RAISE EXCEPTION 'Sem permissão para cancelar mesa';
  END IF;

  SELECT * INTO v_order FROM public.pdv_orders WHERE id = p_order_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Pedido não encontrado'; END IF;

  v_owner := public.pdv_resolve_owner(auth.uid());
  IF v_order.user_id <> v_owner THEN
    RAISE EXCEPTION 'Pedido de outro estabelecimento';
  END IF;

  IF v_order.status IN ('cancelada','fechado') THEN
    RAISE EXCEPTION 'Pedido já finalizado (status: %)', v_order.status;
  END IF;

  -- Bloqueia se houver itens já pagos ou em cobrança
  IF EXISTS (
    SELECT 1
    FROM public.pdv_comanda_items ci
    JOIN public.pdv_comandas c ON c.id = ci.comanda_id
    WHERE c.order_id = p_order_id
      AND (
        COALESCE(ci.paid_quantity, 0) > 0
        OR ci.charging_session_id IS NOT NULL
      )
  ) THEN
    RAISE EXCEPTION 'Mesa possui itens pagos ou em cobrança — não pode ser cancelada';
  END IF;

  -- Cancela todas as comandas filhas que ainda estão ativas
  UPDATE public.pdv_comandas
     SET status = 'cancelada',
         close_reason = COALESCE(p_reason, close_reason),
         closed_by_user_id = auth.uid(),
         closed_by_waiter_at = COALESCE(closed_by_waiter_at, v_now),
         updated_at = v_now
   WHERE order_id = p_order_id
     AND status IN ('aberta','aguardando_pagamento','em_cobranca');

  -- Marca pedido como cancelado
  UPDATE public.pdv_orders
     SET status = 'cancelada',
         cancelled_at = v_now,
         cancellation_reason = COALESCE(p_reason, cancellation_reason),
         updated_at = v_now
   WHERE id = p_order_id;

  -- Libera a mesa caso ainda aponte para este pedido
  UPDATE public.pdv_tables
     SET current_order_id = NULL,
         status = 'livre',
         updated_at = v_now
   WHERE current_order_id = p_order_id;

  PERFORM public.log_pdv_action(
    'close_attendance', 'order', p_order_id, 'order', p_order_id,
    jsonb_build_object('cancelled', true),
    p_reason
  );

  RETURN jsonb_build_object('ok', true, 'order_id', p_order_id);
END;
$function$;